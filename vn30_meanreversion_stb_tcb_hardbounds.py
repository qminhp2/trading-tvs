...
df_tcb = load_clean_csv('data/tcb_ver2.csv')
df_stb = load_clean_csv('data/stb_ver2.csv')
df_vn30 = load_clean_csv('data/vn30_ver2.csv')

# align to common date range to avoid holes / phantom bars
start = max(df_tcb.index.min(), df_stb.index.min(), df_vn30.index.min())
end   = min(df_tcb.index.max(), df_stb.index.max(), df_vn30.index.max())
df_tcb = df_tcb.loc[start:end]
df_stb = df_stb.loc[start:end]
df_vn30 = df_vn30.loc[start:end]

for df in [df_tcb, df_stb, df_vn30]:
    df.sort_values('Date', inplace=True)
    df.replace([np.inf, -np.inf], np.nan, inplace=True)   # guard against log/parse artifacts
    df.dropna(subset=['Close'], inplace=True)

common_idx = df_tcb.index.intersection(df_stb.index).intersection(df_vn30.index)
df_tcb = df_tcb.loc[common_idx].copy()
df_stb = df_stb.loc[common_idx].copy()
df_vn30 = df_vn30.loc[common_idx].copy()

# final sanity before feeding Backtrader (cheap & effective)
assert df_tcb['Close'].notna().all() and df_stb['Close'].notna().all() and df_vn30['Close'].notna().all()

# Johansen cointegration test
df_log = pd.concat([
    np.log(df_tcb['Close']).rename('log_TCB'),
    np.log(df_stb['Close']).rename('log_STB'),
...
class LogSpread(bt.Strategy):
    ...
    def __init__(self):
        self.tcb, self.stb, self.vn30 = self.datas
        self.tcb_close = self.tcb.close
        self.stb_close = self.stb.close
    ...
    def in_position(self):
        ...
        return any(self.getposition(d).size != 0 for d in (self.tcb, self.stb, self.vn30))

    def flat_now(self):
        return all(self.getposition(d).size == 0 for d in (self.tcb, self.stb, self.vn30))
    ...
    def next(self):
        prices = np.array([
            np.log(self.tcb_close[0]),
            np.log(self.stb_close[0]),
        ...
        base = max(1, int(0.1 * self.broker.getvalue() / 
                    (self.tcb_close[0] + self.stb_close[0] + abs(self.vn30_close[0]))))

        # Notional hedge for VN30 leg (keeps code simple, no new func)
        vn30_sz = int(round(
            base * abs(self.vector[2]) *
            ((self.tcb_close[0] + self.stb_close[0]) / max(1e-12, self.vn30_close[0]))
        ))
        ...
        if not self.in_position():
            ...
            if zscore < self.p.z_score_long_entry:
                # Oversold -> LONG spread: +TCB +STB -VN30
                self.log('SIGNAL: ENTER LONG SPREAD (+TCB +STB -VN30)')
                self.position_direction = +1
                self.open_orders += [
                    self.buy(data=self.tcb, size=base),
                    self.buy(data=self.stb, size=base),
                    self.sell(data=self.vn30, size=vn30_sz),
                ]

            elif zscore > self.p.z_score_short_entry:
                # Overbought -> SHORT spread: -TCB -STB +VN30
                self.log('SIGNAL: ENTER SHORT SPREAD (-TCB -STB +VN30)')
                self.position_direction = -1
                self.open_orders += [
                    self.sell(data=self.tcb, size=base),
                    self.sell(data=self.stb, size=base),
...
if __name__ == '__main__':
    ...
    data0 = bt.feeds.PandasData(dataname=df_tcb, name='TCB')
    data1 = bt.feeds.PandasData(dataname=df_stb, name='STB')
